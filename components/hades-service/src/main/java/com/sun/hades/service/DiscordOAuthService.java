...
/**
 * Discord OAuth2 code exchange, guild gate, and role-to-CEFR resolution.
 */
@Service
public class DiscordOAuthService {
   ...
   * @return the verified Discord profile, with CEFR level from guild roles
   */
  public DiscordProfile exchange(String code) {
    ...
    CefrLevel level = resolveCefrLevel(accessToken);
  ...
  private CefrLevel resolveCefrLevel(String accessToken) {
    Map<?, ?> member = rest.get()
    ...
    List<?> roles = rolesRaw instanceof List<?> list ? list : List.of();
    List<String> roleIds = roles.stream()
        .map(String::valueOf)
        .collect(Collectors.toList());
    Map<String, String> roleNames = guildRoleNames();
    return roleIds.stream()
        .map(roleNames::get)
        .filter(name -> name != null && toLevel(name).isPresent())
        .map(name -> toLevel(name).get())
        .max(Comparator.comparingInt(Enum::ordinal))
        .orElse(null);
  }

  private Map<String, String> guildRoleNames() {
    if (botToken == null || botToken.isBlank()) {
      return Map.of();
    }
    List<?> roles = rest.get()
        .uri(BASE + "/guilds/" + guildId + "/roles")
    ...
    return roles.stream()
        .collect(Collectors.toMap(
            r -> String.valueOf(((Map<?, ?>) r).get("id")),
            r -> String.valueOf(((Map<?, ?>) r).get("name"))));
  }

  private Optional<CefrLevel> toLevel(String name) {
    return Arrays.stream(CefrLevel.values())
