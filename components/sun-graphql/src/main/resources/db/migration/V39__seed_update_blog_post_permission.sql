-- V39 seeds graphql.briareus.updateBlogPost permission anchored on createBlogPost.

INSERT INTO gaia_role_permissions (id, role_id, permission, createdat, lastupdatedat)
SELECT gen_random_uuid(), rp.role_id, 'graphql.briareus.updateBlogPost', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
SELECT gen_random_uuid(), ap.account_id, 'graphql.briareus.updateBlogPost', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
WHERE rp.permission = 'graphql.briareus.listBlogPosts'
  AND NOT EXISTS (
    ...
    WHERE rp2.role_id = rp.role_id AND rp2.permission = 'graphql.briareus.updateBlogPost'
  )
ON CONFLICT (role_id, permission) DO NOTHING;
...
WHERE ap.permission = 'graphql.briareus.listBlogPosts'
  AND NOT EXISTS (
    ...
    WHERE ap2.account_id = ap.account_id AND ap2.permission = 'graphql.briareus.updateBlogPost'
  )
ON CONFLICT (account_id, permission) DO NOTHING;
